...
//////////////////////////////////////////
    // layerFow (map: Phaser.Tilemaps.Tilemap, tileset: string) {
    //     this.map = map;
    
    //     var layer = this.map.createBlankLayer('layerFow', tileset);
    //     //layer.setScale(1);
    //     layer.visible;
    //     let tileCallback = () => {
    //         //layer.alpha = 0.5;
    //         layer.fill(50);
    //     }
    //     layer.forEachTile(tileCallback)
...
export default class FowLayer{
    ...
    let setVisibility = (x:number, y:number) => {
        ...
        if (d > radius  && this.isSeen[x][y] === true) { //   && this.isSeen[x][y] == true
            tile.setAlpha(0.5);
            //this.layerFow (this.map, 'tiles');
        } else {
            tile.setAlpha(1);
        }
